...
const Movies = () => {
  ...
  return (
    <div className="bg-white py-8 px-2 md:px-16">
      <div>
        <h1 className="text-2xl font-semibold text-neutral border-l-4 border-primary pl-4 mb-5">"Popular Movies"</h1>
      </div>
      ...
      {movies.length === 0 ? (
        ...
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-7 gap-x-3 gap-y-6">
            {movies.map((item) => {
              // console.log(item);
                  ...
                  <Link to={`/movie/${id}`} className="flex flex-col">
                    <div className="relative w-full min-h-[350px]">
                      ...
                      <span className="absolute -bottom-3 left-2 w-10 h-10 rounded-full bg-neutral  flex items-center justify-center">
                        <span
                          className={`rounded-full w-9 h-9 bg-neutral text-white font-semibold flex items-center justify-center border-2 p-2 ${rating.average > 5 ? "border-success" : "border-error"
                            }`}
                        >
                          {rating.average}
                        </span>
